-- 1. Drop the policies FIRST to remove column dependencies
drop policy if exists "Users can manage their own prep kits" on public.prep_kits;
drop policy if exists "Users can manage their own prep kits" on prep_kits;
...
drop policy if exists "Users can view/write chat messages under their sessions" on chat_messages;
drop policy if exists "Users can manage embeddings linked to their own resumes" on public.resume_embeddings;
drop policy if exists "Users can manage embeddings linked to their own resumes" on resume_embeddings;
...
-- 2. Drop foreign key constraints on user_id
